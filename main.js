...
function StereoCamera(
    ...
    this.ApplyLeftFrustum = function (matrix) {
        ...
        let modelProjectionMatrix = m4.multiply(this.mProjectionMatrix, m4.multiply(this.mModelViewMatrix, matrix));
        if (gl) {
            gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, modelProjectionMatrix)
            gl.colorMask(true, false, false, false);
        }
    }

    this.ApplyRightFrustum = function (matrix) {
        let top, bottom, left, right;
...
function draw() {
    ...
    camera3D.ApplyRightFrustum(matAccum1)
    surface.Draw();
...
function init() {
    ...
    plane.NormalBufferData(buildPlaneBufferData())
    plane.TextureBufferData(buildPlaneTextureBufferData())
    camera3D = new StereoCamera(0.5, 0.01, 1, 1, 7, 13)
    ui = new dat.GUI()
    ui.add(camera3D, 'mConvergence', 0.5, 10)
...
function buildPlaneTextureBufferData() {
    const vertices = [
        [1, 1],
        [0, 0],
        [0, 1],
